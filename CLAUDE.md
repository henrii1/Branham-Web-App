...
**Native share** (`ShareModal`'s "Share directly" button, below Download Image) hands the rendered card off to the OS share sheet via `navigator.share({ files, title, text, url })`, so the user picks WhatsApp/Instagram/Messages/etc. themselves — the web platform has no way for a site to target one specific app directly (see `docs/superpowers/specs/2026-07-30-native-share-design.md` for why: no public web share-intent exists for posting to Instagram/TikTok, and Facebook's own share dialog only ever pulls `og:image`, never a custom upload). Only renders when `navigator.share`/`navigator.canShare` exist (mobile Safari/Chrome; most desktop browsers never show it). Because `navigator.share()` must fire within a live user-activation window and `html-to-image` rasterization is too slow to do on click, the PNG is generated **proactively**, debounced ~400ms after `format`/`backgroundIndex`/`textShadeIndex` settle, with a monotonic generation token guarding against a stale slow render overwriting a newer one — the button stays disabled/"Generating…" until that Blob exists. `Download Image` opportunistically reuses the same Blob when it's fresh, instead of rasterizing twice. `AbortError` (user backed out of the share sheet) is a silent no-op, not an error.
